...
export function CityAccessProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchCitiesFromAPI = useCallback(async (userObj: any): Promise<{ cities: string[], isAdmin: boolean } | null> => {
    try {
      ...
      if (resp.ok) {
        ...
        // 🚨 CRITICAL: Validate tenant_id if provided
  ...
  const fetchCityAccess = useCallback(async (forceRefresh = false) => {
    // 🔥 CACHE BYPASS: Check for nocache URL parameter
    ...
    const fetchRequest = (async () => {
      ...
      try {
        ...
        if (result) {
          ...
          // Save to caches (unless cache bypass is active)
          if (!cacheBypass) {
  ...
  // 🔒 CRITICAL: Detect tenant changes and invalidate cache
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // ✅ FIX: Wait for tenant_id before fetching cities (prevents race condition)
