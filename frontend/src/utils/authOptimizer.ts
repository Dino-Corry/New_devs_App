...
class AuthOptimizer {
  ...
  async getSession(forceRefresh: boolean = false): Promise<any> {
    // CRITICAL: Don't return cached session or fetch new session during logout
    ...
    // CRITICAL FIX: For initial load, check if we need to recover session first
  ...
  private async fetchSessionWithTimeout(): Promise<any> {
    // CRITICAL: Don't fetch session during logout
    if (typeof window !== 'undefined' && (window as any).__isLoggingOut) {
    ...
    const fetchPromise = (async () => {
      ...
      try {
        // CRITICAL: On page refresh, Supabase needs a moment to initialize and load session from storage
