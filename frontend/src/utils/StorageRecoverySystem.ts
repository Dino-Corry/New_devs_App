 ...
 * - Handles emergency storage reset while preserving critical data
...
export class StorageRecoverySystem {
  ...
  async attemptRecovery(healthReport?: StorageHealthReport): Promise<RecoveryReport> {
    ...
    try {
      ...
      // Restore critical data if recovery was aggressive
  ...
  private selectRecoveryStrategies(issues: StorageIssue[]): RecoveryStrategy[] {
    ...
    for (const strategy of this.recoveryStrategies) {
      ...
      if (isApplicable) {
        // For critical issues, include more aggressive strategies
  ...
  /**
   * Backup critical data before aggressive recovery
   */
  ...
  /**
   * Restore critical data after recovery
   */
  ...
  private async emergencyResetWithBackup(): Promise<boolean> {
    try {
      ...
      // Clear all storage except emergency backup
