...
class LocalStorageManager {
  ...
  private migrateV1ToV2(): void {
    try {
      ...
      // CRITICAL: Preserve ALL Supabase auth keys (they use dynamic keys based on project ID)
      ...
      essentialKeys.forEach(key => {
        const value = localStorage.getItem(key);
        if (value) {
          // Validate JSON structure for critical keys
  ...
  private clearAllExceptEssentials(): void {
    ...
    // CRITICAL: Preserve ALL Supabase auth keys
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && (key.includes('supabase') || key.includes('sb-'))) {
        const value = localStorage.getItem(key);
