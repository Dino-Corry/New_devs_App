...
# Emergency admin fallback cities by tenant - ONLY used when admin has no other city access
...
async def get_all_tenant_cities(tenant_id: str) -> List[str]:
    ...
    # PERFORMANCE FIX: Add Redis health check and graceful degradation
    redis_healthy = False
    if redis_client:
        try:
    ...
    try:
        ...
        # PERFORMANCE FIX: Add query timeout and simpler query
...
async def get_city_access_fast(
    ...
    try:
        ...
        # 🔒 CRITICAL: Strict tenant validation - fail fast instead of returning empty
            ...
            if is_admin:
                ...
                try:
                    ...
                    if result.error:
                        ...
                        # Emergency fallback only if truly no properties exist for this tenant
                        if not cities:
                    ...
                    # Emergency fallback on database error
                    emergency_cities = TENANT_EMERGENCY_CITIES.get(tenant_id, [])
